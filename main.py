...
ADMIN_PASSWORD = os.environ.get("ADMIN_PASSWORD", "KT_WgPtce4")
SESSION_SECRET = os.environ.get("SESSION_SECRET", "cartridge-secret-key-2024")
...
async def register(
    ...
    if password != password_confirm:
        return RedirectResponse(url="/register?error=" + quote("Пароли не совпадают"), status_code=303)
    
    existing_user = db.query(User).filter(User.email == email).first()
    if existing_user:
        return RedirectResponse(url="/register?error=" + quote("Пользователь с такой почтой уже существует"), status_code=303)
    ...
    if existing_pending:
        return RedirectResponse(url="/register?error=" + quote("Заявка на регистрацию уже подана"), status_code=303)
    ...
    return RedirectResponse(url="/register?success=" + quote("Заявка отправлена! Ожидайте одобрения администратора."), status_code=303)


@app.get("/admin/registrations", response_class=HTMLResponse)
async def admin_registrations(request: Request, db: Session = Depends(get_db)):
    if not request.session.get("is_admin"):
        return RedirectResponse(url="/", status_code=303)
    
    pending = db.query(PendingRegistration).filter(PendingRegistration.status == "pending").all()
...
@app.post("/admin/registrations/{reg_id}/approve")
async def approve_registration(reg_id: int, request: Request, db: Session = Depends(get_db)):
    if not request.session.get("is_admin"):
        return RedirectResponse(url="/", status_code=303)
    
    pending = db.query(PendingRegistration).filter(PendingRegistration.id == reg_id).first()
    if pending:
        user = User(
            email=pending.email,
